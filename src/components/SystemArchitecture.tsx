...
import { Layers, Cpu, Shield, Zap, BrainCircuit, ArrowRight, Terminal, CheckCircle2 } from 'lucide-react';
  ...
  const layers: ArchitectureLayer[] = [
    {
      ...
      description: 'Deploy-ready core optimized for sub-500ms TTFB, zero-latency hydration, and search ranking dominance.',
...
export default defineConfig({
  ...
  optimization: {
    ...
    },
    {
      ...
      description: 'Fluid page transitions, physics-based gesture curves, and hardware-accelerated layouts designed to retain user traffic.',
...
export const pageTransition = {
  ...
  exit: { opacity: 0 }
    ...
    },
    {
      ...
      description: 'Offline-first database gateways and vector caching optimized for sub-millisecond retrieval speeds.',
...
return cache;`
    },
    {
      ...
      description: 'Rate-limiting rules, automated request scrubbing, end-to-end payload encryption, and penetration-tested API endpoints.',
...
export const middleware = (req) => 
  rateLimiter.verify(req);`
    },
    {
      ...
      description: 'Serverless neural models that pre-qualify user requirements, analyze engagement profiles, and execute agentic routing.',
  ...
  const getAccentColor = (color: string) => {
    switch (color) {
      case 'blue': return 'text-blue-500 bg-blue-500/[0.03] border-blue-500/20';
      case 'purple': return 'text-purple-500 bg-purple-500/[0.03] border-purple-500/20';
      case 'red': return 'text-red-500 bg-red-500/[0.03] border-red-500/20';
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="text-center mb-12 sm:mb-20">
          ...
          <p className="text-zinc-400 text-base sm:text-xl max-w-xl mx-auto leading-relaxed font-medium">
            Explore the proprietary, zero-latency layers we integrate to secure, automate, and scale your digital operations.
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-12 gap-8 items-stretch">
          ...
          <div className="lg:col-span-6 space-y-4">
            <div className="text-[10px] font-black uppercase tracking-widest text-zinc-500 mb-2">Layer Stack Selection</div>
            ...
            <div className="space-y-3">
              {layers.map((layer) => {
                ...
                return (
                  <button
                    key={layer.id}
                    onClick={() => { playClick(); setActiveLayerId(layer.id); }}
                  ...
                  >
                    <div className={`w-8 h-8 rounded-lg border flex items-center justify-center shrink-0 transition-colors ${
                      isActive ? 'bg-blue-600 border-blue-500 text-white' : 'border-white/20 bg-white/[0.02]'
                    }`}>
                      {layer.icon}
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="text-sm font-bold tracking-tight mb-0.5 truncate">{layer.name}</div>
                      <div className="text-xs text-zinc-500 font-mono truncate">{layer.tech}</div>
                    </div>
                    <div className={`text-[9px] font-black font-mono shrink-0 transition-colors ${
          ...
          <div className="lg:col-span-6 flex flex-col justify-between p-6 sm:p-10 rounded-[2rem] border border-white/10 bg-[#090a0d] relative overflow-hidden">
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-start">
                <div>
                  <span className="text-[9px] font-black uppercase tracking-[0.4em] text-zinc-500 block font-mono">{activeLayer.tag}</span>
                  <h4 className="text-xl font-black text-white uppercase tracking-tight mt-1">{activeLayer.name}</h4>
                </div>
                <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 border ${getAccentColor(activeLayer.color)}`}>
              ...
              <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/5 space-y-4">
                <div className="space-y-2">
                  <div className="flex justify-between text-xs font-mono">
                    <span className="text-zinc-500 font-bold uppercase tracking-wider">{activeLayer.metricLabel}</span>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-xs font-mono">
                    <span className="text-zinc-500 font-bold uppercase tracking-wider">{activeLayer.metric2Label}</span>
              ...
              <div className="rounded-xl border border-white/5 bg-black/60 p-4 font-mono text-[10px] leading-relaxed text-zinc-300 relative overflow-hidden">
                <div className="absolute top-2 right-3 flex items-center gap-1.5 opacity-30 select-none">
                  <Terminal size={10} />
                  <span className="text-[8px] uppercase tracking-widest font-black">Protocol Specs</span>
                </div>
