...
import { results, clientTestimonials } from '../data/portfolioData'
import { useAudioUI } from '../context/AudioUIContext'

const accentMap: Record<
  string,
  { bg: string; border: string; text: string; glow: string; accentColor: string }
> = {
  emerald: {
        ...
        <div className='grid md:grid-cols-3 gap-4 mb-12'>
          {clientTestimonials.map((testimonial, idx) => (
            <motion.div
              ...
              className='p-6 rounded-2xl bg-zinc-900/30 border border-white/5 relative'
            >
          ...
          <AnimatePresence mode='wait'>
            ...
            >
              ...
              <div className='flex items-start justify-between mb-8'>
                <div
                  className={`w-12 h-12 rounded-2xl ${activeAccent.bg} flex items-center justify-center ${activeAccent.text}`}
                >
                  {activeResult.icon}
                </div>
                <div className='text-right'>
                  <div
                    className={`text-3xl sm:text-4xl font-black tracking-tighter ${activeAccent.text}`}
                  >
                    {activeResult.metric}
                  </div>
                  <div className='text-[9px] font-bold text-zinc-600 uppercase tracking-widest'>
                    {activeResult.metricLabel}
                  </div>
                </div>
              </div>
              ...
              {/* Hover arrow */}
              <div
                className={`mt-6 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.3em] ${activeAccent.text} opacity-0 group-hover:opacity-100 transition-opacity duration-300`}
              >
              ...
              {results.map((_, idx) => (
                <button
                  ...
                  className={`h-1.5 rounded-full transition-all duration-300 cursor-pointer ${
                    currentIdx === idx
                      ? `w-6 ${activeAccent.bg.replace('bg-', 'bg-').split('/')[0]}`
                      : 'w-1.5 bg-white/10'
                  }`}
